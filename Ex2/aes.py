...
0x28, 0xdf, 0x8c, 0xa1, 0x89, 0x0d, 0xbf, 0xe6, 0x42, 0x68, 0x41, 0x99, 0x2d, 0x0f, 0xb0, 0x54, 0xbb, 0x16]
INV_SBOX = [0]*256
...
def key_expansion(key):
    ...
    for i in range(16, 176, 4):
        ...
        if i % 16 == 0:
            ...
            t[0] ^= RCON[(i//16)-1]
        for j in range(4): w.append(w[i+j-16] ^ t[j])
    return w

def mix_cols(s, inv=False):
    ns = [0]*16
    for c in range(4):
        i, a = c*4, s[c*4:c*4+4]
        if not inv:
            ns[i]=mul(a[0],2)^mul(a[1],3)^a[2]^a[3]; ns[i+1]=a[0]^mul(a[1],2)^mul(a[2],3)^a[3]
            ns[i+2]=a[0]^a[1]^mul(a[2],2)^mul(a[3],3); ns[i+3]=mul(a[0],3)^a[1]^a[2]^mul(a[3],2)
        else:
            ns[i]=mul(a[0],14)^mul(a[1],11)^mul(a[2],13)^mul(a[3],9); ns[i+1]=mul(a[0],9)^mul(a[1],14)^mul(a[2],11)^mul(a[3],13)
            ns[i+2]=mul(a[0],13)^mul(a[1],9)^mul(a[2],14)^mul(a[3],11); ns[i+3]=mul(a[0],11)^mul(a[1],13)^mul(a[2],9)^mul(a[3],14)
    return ns

def aes_main(state, key, decrypt=False):
    exp = key_expansion(key)
    if not decrypt:
        state = [state[i]^exp[i] for i in range(16)]
        for r in range(1, 10):
            state = [SBOX[b] for b in state]
            state = [state[i] for i in [0,5,10,15,4,9,14,3,8,13,2,7,12,1,6,11]]
            state = mix_cols(state)
            state = [state[i]^exp[r*16+i] for i in range(16)]
        state = [SBOX[b] for b in state]
        state = [state[i] for i in [0,5,10,15,4,9,14,3,8,13,2,7,12,1,6,11]]
        return [state[i]^exp[160+i] for i in range(16)]
    else:
        state = [state[i]^exp[160+i] for i in range(16)]
        for r in range(9, 0, -1):
            state = [state[i] for i in [0,13,10,7,4,1,14,11,8,5,2,15,12,9,6,3]]
            state = [INV_SBOX[b] for b in state]
            state = [state[i]^exp[r*16+i] for i in range(16)]
            state = mix_cols(state, True)
        state = [state[i] for i in [0,13,10,7,4,1,14,11,8,5,2,15,12,9,6,3]]
        state = [INV_SBOX[b] for b in state]
        return [state[i]^exp[i] for i in range(16)]

# Main Execution
choice = input("AES: Type 'E' to Encrypt or 'D' to Decrypt: ").upper()
key = input("Enter 16-char key: ").ljust(16)[:16].encode()

if choice == 'E':
    msg = input("Enter 16-char message: ").ljust(16)[:16].encode()
    cipher = aes_main(list(msg), list(key), False)
    print("Cipher (Hex):", "".join(f"{b:02x}" for b in cipher))
elif choice == 'D':
    hex_msg = input("Enter Hex ciphertext: ")
    cipher_bytes = [int(hex_msg[i:i+2], 16) for i in range(0, len(hex_msg), 2)]
    plain = aes_main(cipher_bytes, list(key), True)
    print("Plaintext:", "".join(chr(b) for b in plain).strip('\0'))
